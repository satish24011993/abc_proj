...
from management_app.models import Partner
from task_project.utils.mail import send_mail_async as send_mail
...
class Task(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='task_assigned', verbose_name=_('assigned to'), on_delete=models.SET_NULL, null=True, blank=True)
    ...
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='users_created', verbose_name=_('created by'), on_delete=models.SET_NULL, null=True)
    ...
    def send_new_task_email(self):
        ...
        if len(emails_to):
            ...
            if settings.TASKS_VIEWER_ENABLED:
                email_template = settings.TASKS_APP_EMAIL_WITHOUT_URL
                vals["url"] = self.get_task_viewer_url()
            else:
                email_template = settings.TASKS_APP_EMAIL_WITHOUT_URL
            try:
                send_mail(
                ...
                logger.warning("[Task #%s] Error trying to send the task creation email - %s: %s", self.number, e.__class__.__name__, str(e))

    def get_tasks_viewer_url(self):

        """
        Verification token added to the Tasks Viewer URL so each one sent through email connot be used to change the other number and access to other orders.

        It uses as input a salt code configured and the ID number.

        See: task_project/settings_emails.py
        """
        ...
        #created_at_as_iso = self.created_at.strftime("%Y-%m-%dT%H:%M:%S.%fZ") # This ISO format is the same used by the REST serializer
