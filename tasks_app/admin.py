...
class TaskAdmin(AdminAdvancedFiltersMixin, admin.ModelAdmin):
    ...
    search_fields = ('id', 'title', 'item__item_description', 'user__username', 'user__first_name', 'user__last_name','partner__name','partner__email')
    list_filter = (
    ...
    fieldsets = (
        (None,{'fields': ('title', ('user', 'partner'), 'deadline',('state', 'priority'), ('description', 'resolution'))}),(_('More...'), {'fields': (('created_at', 'last_modified'), 'created_by'), 'classes':('collapse',)}),
    )
    inlines = [ItemInline]
